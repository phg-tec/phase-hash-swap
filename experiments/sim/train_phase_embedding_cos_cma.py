...
def generate_training_dataset(dim: int = 256,
                              ...
                              rng_seed: int = 123):
    """
    Dataset con pares (x,y) cubriendo cosenos en [-1,1].
    """
    rng = np.random.default_rng(rng_seed)
    cos_values = np.linspace(-1.0, 1.0, num=21)  # -1, -0.9, ..., 1

    X_list = []
    Y_list = []

    for _ in range(n_pairs):
        c = rng.choice(cos_values)
...
def evaluate_params(params, X, Y, backend, shots=2048):
    """
    Función objetivo para CMA-ES.
    params: vector de parámetros del MLP(cos), longitud N_PARAMS_MLP
    X, Y: dataset (N,dim)
    Devuelve MSE sobre el dataset entre p0_real y p0_quantum.
    """
    N = X.shape[0]
    sq_errors = []
...
def main():
    dim = 128          # dimensión de los vectores de entrenamiento (potencia de 2)
    n_pairs = 250      # nº de pares de entrenamiento
    shots = 4096
    ...
    # parámetros iniciales: pequeños valores aleatorios
    ...
    def objective(theta_vec):
        return evaluate_params(theta_vec, X, Y, backend=backend, shots=shots)

    print("Comenzando optimización CMA-ES (MLP(cos)-dependiente)...")
    while not es.stop():
        solutions = es.ask()
